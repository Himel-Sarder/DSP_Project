...
%% =============================== LOW-PASS FILTER ==============================================
disp('Applying Low-pass Filter...');           % Inform about filtering process
Fc_low = 1500;                                 % Cutoff frequency = 1500 Hz
order_low = 6;                                 % Filter order = 6
[b_low, a_low] = butter(order_low, Fc_low/(Fs/2), 'low'); % Design Butterworth low-pass filter
y_low = filter(b_low, a_low, y);              % Apply filter to audio

disp('Playing Low-pass Filtered Audio...');    
sound(y_low, Fs);                              % Play low-pass filtered audio
pause(length(y)/Fs + 1);                       % Pause until playback finishes

%% =============================== HIGH-PASS FILTER =============================================
disp('Applying High-pass Filter...');          % Notify user
Fc_high = 1000;                                % High-pass cutoff = 1000 Hz
order_high = 6;                                % Filter order
[b_high, a_high] = butter(order_high, Fc_high/(Fs/2), 'high'); % Design high-pass filter
y_high = filter(b_high, a_high, y);           % Apply high-pass filter

disp('Playing High-pass Filtered Audio...');
sound(y_high, Fs);                             % Play high-pass filtered audio
pause(length(y)/Fs + 1);

%% =============================== BAND-PASS FILTER =============================================
disp('Applying Band-pass Filter...');
Fc1 = 500; Fc2 = 2000;                         % Band-pass range = 500 Hz to 2000 Hz
order_band = 6;                                % Filter order
[b_band, a_band] = butter(order_band, [Fc1 Fc2]/(Fs/2), 'bandpass'); % Design band-pass filter
y_band = filter(b_band, a_band, y);           % Apply band-pass filter

disp('Playing Band-pass Filtered Audio...');
sound(y_band, Fs);                             % Play filtered audio
pause(length(y)/Fs + 1);

%% =============================== HIGH-PASS FILTER FOR DENOISING ===============================
disp('Applying High-pass Filter for Denoising...');
Fc_denoise = 1200;                             % Cutoff frequency for denoising
order_denoise = 6;                             
[b_denoise, a_denoise] = butter(order_denoise, Fc_denoise/(Fs/2), 'high'); % Design high-pass filter

% Apply high-pass filter
y_temp = filter(b_denoise, a_denoise, y);      % Filtered audio to remove low-frequency noise

% Wiener filter for adaptive noise removal
y_denoised = wiener2(y_temp, [5 1]);           % Apply Wiener filter for smoothing and denoising

disp('Playing Denoised Audio...');
sound(y_denoised, Fs);                         % Play denoised audio
pause(length(y)/Fs + 1);
...
subplot(5,1,5);
plot(t, y_denoised, 'g');                      % Denoised audio
...
noise_after  = y - y_denoised;                  % Estimate noise after denoising
...
Y_denoised_fft = fft(y_denoised);      % FFT of denoised audio
